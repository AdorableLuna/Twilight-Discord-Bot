...
class ApplicationsRaid(Maincog):

    def __init__(self, client):
        Maincog.__init__(self, client, whitelistedChannels = [862692134330564668])
    ...
    async def on_ready_init(self):
        ...
        self.acceptMessage = (f"{self.twilightEmoji} **Twilight Application Accepted: Raid** {self.twilightEmoji}\n\n"

        "After careful consideration, we have decided to accept your application to join Twilight as a Raid Booster. Down below you will find a list of things to do before you start boosting in our community.\n\n"
        ...
        self.declineMessage = (f"{self.twilightEmoji} **Twilight Application Declined: Raid** {self.twilightEmoji}\n\n"

        "We're sorry to inform you that we have declined your Raid Application over at Twilight. Feel free to re-apply when your logs increased sufficiently in regards to the requirements.")
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if response.ok:
            ...
            raidRole = self.helper.getRole(guild, f"SoD {data['faction'].capitalize()} Booster")
        ...
        if payload.member.mention in access['raids']['users']:
            ...
            if str(payload.emoji) == str(self.acceptEmoji):
                ...
                self.client.sheet.add(SPREADSHEET_ID, "'Applications'!B3:D", [f"{data['name']}-{data['realm'].replace(' ', '')}", data['realm'].replace(' ', ''), data['faction'].capitalize()])
    ...
    async def on_message(self, message):
        ...
        try:
            ...
            if response.ok:
                ...
                embed.set_author(name=f"{data['faction'].capitalize()} Raid Application", icon_url=str(message.author.avatar_url))
...
def setup(client):
    client.add_cog(ApplicationsRaid(client))
